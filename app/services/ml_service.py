...
class MLService:
    ...
    async def generate_user_embeddings(self, user_id: int) -> np.ndarray:
        ...
        # 3. Combine features
        feature_vector[50:100] = list(course_prefs.values())[:50] if course_prefs else np.zeros(50)
        feature_vector[100:150] = list(tag_prefs.values())[:50] if tag_prefs else np.zeros(50)
    ...
    async def generate_video_embeddings(self, video_id: int) -> np.ndarray:
        ...
        # 1. Metadata features
        # Title length, description length, etc.
        ...
        engagement_score = (video.like_count * 2 + video.comment_count * 3 + video.share_count * 4) / total_interactions
    ...
    async def find_similar_users(self, user_id: int, limit: int = 10) -> List[Tuple[int, float]]:
        ...
        for emb in other_embeddings:
            ...
            similarities.append((emb.user_id, similarity))
    ...
    async def find_similar_videos(self, video_id: int, limit: int = 10) -> List[Tuple[int, float]]:
        ...
        for emb in other_embeddings:
            other_vector = np.array(emb.embedding_vector)
            similarity = cosine_similarity([target_vector], [other_vector])[0][0]
            similarities.append((emb.video_id, similarity))
    ...
    async def compute_engagement_score(self, user_id: int) -> float:
        ...
        watch_time_score = min(stats.total_watch_time / 36000, 1.0) * 35  # 10 hours max
    ...
    async def _calculate_ndcg(self, feedback: List, k: int) -> float:
        ...
        ideal_relevance = sorted([1 if f.clicked == 1 else 0 for f in feedback], reverse=True)[:k]
        idcg = 0.0
